  ...
  const getOrders = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.orders)) {
        setOrders(data.orders); // Ensure only the orders array is set to state
      } else {
        console.error("Orders fetch was successful but no orders were returned.");
        setOrders([]); // Set to empty array if no orders are present
      }
    } catch (error) {
